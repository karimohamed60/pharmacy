...
const PrescriptionsDetails = () => {
  ...
  useEffect(() => {
    ...
    medicinesData.forEach((medicine) => {
      initialCheckedMedicines[medicine.Pmedicine_name] = medicine.got_medicine === 1;
    });
...
;
  ...
  const handlePrint = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/students/${studentId}/prescriptions/${prescription_id}/generate_pdf`, {
        method: "GET",
        headers: {
...
>
  <option style={{color:"transparent"}} >{window.Prescription_status } </option>
...
<div class="checkbox-wrapper-31">
  <input type="checkbox"
        ...
        onChange={(e) => handleCheckboxChange(item.Pmedicine_name, e.target.checked)}
  />
