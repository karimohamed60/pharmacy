...
const MedicineList = () => {
 ...
 /*  useEffect(() => {
  ...
  const handleSearch = async (value, page = 1) => {
    try {
      ...
      const response = await fetch(`${API_URL}/medicines/search?q=${value}`, {
        method: "GET",
        headers: {
   ...
   //   console.log("Response Data:", data); // Log the response data
      ...
      const results = data.data.filter((medicine) => {
        return (
          value &&
          medicine &&
          medicine.attributes.ingredient_name &&
          medicine.attributes.ingredient_name
            .toLowerCase()
            .includes(value.toLowerCase()) ||
          medicine.attributes.commercial_name
            .toLowerCase()
            .includes(value.toLowerCase())
        );
      });
    // console.log("Filtered Results:", results); // Log the filtered results
           setMedicines(data.data); // Update medicines list

      const totalMedicines = data['total_medicines'] // Assuming total_students is the correct key
      const totalPages = Math.ceil(totalMedicines / recordsPerPage);
  ...
  const getMedicines = async () => {
    try {
      ...
      const response = await fetch(
       ` ${API_URL}/medicines?per_page=${recordsPerPage}&page=1`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
 ...
 const fetchMedicines = async (page, token) => {
    try {
      const response = await fetch(`
      ${API_URL}/medicines?per_page=${recordsPerPage}&page=${page}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const fetchedMedicines = await response.json();
      return fetchedMedicines.data;
    } catch (error) {
  ...
  const handlePageClick = async (data) => {
    try {
      ...
      const fetchedMedicines = await fetchMedicines(currentPage, token, search);
      setMedicines(fetchedMedicines);
  
      if (fetchedMedicines.length =10) {
        setTotalPages(1);
      }
    } catch (error) {
...
*/
  ...
  return (
    <>
      ...
      <Modal show={showModal} onHide={handleCloseModal} size="lg" style={{marginTop:'15rem'}}>
          ...
          <div className="ao-row p-5">
            <div className="ao-form-group">
              <div className="aoo-input-group ">
                <label className="ao-label ao-national national_idLabel">National ID:</label>
...
/>
            ...
            <div className="ao-form-group">
              <div className="aoo-input-group">
                ...
                <input className="ao-input mb-2 toleft" type="text" disabled  value={studentName}/>
              </div>
            </div>
          </div>
...
<input
  ...
  onChange={(e) => handleQuantityChange(index, e.target.value)} // Pass index and new quantity
/>
...
/* to add more than one medicine in the order 
